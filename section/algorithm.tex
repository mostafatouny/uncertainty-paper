...
M^{2^k}[i,j] &= \begin{cases} 
    q & A^{[2^{k-1}]}[i,q] = 1 = A^{[2^{k-1}]}[q,j], \; i,j,q \text{ distinct,} \\
      Nil & \text{otherwise.}
    \end{cases}
\end{align*}
...
\textbf{Lemma.} Correctness of $intermediaryWalk$.

Fix an arbitrary graph $G$ with vertices $i$ and $j$. We show for all powers of two $2^k$, if there is a walk of length at most $2^k$, the function $intermediaryWalk(i,j,2^k)$ constructs a correct intermediary walk. The proof is by induction on $k$, the power of walk length.

Base case $k = 0$. Possible walks are of length at most $2^0 = 1$. If $i$ and $j$ are adjacent then $()$ is returned. If not adjacent, then the existing intermediary walk is of length $1$. Call it $(q)$ and note $q = M^{2^k}[i,j]$. Since the intermediary walk is of length $1$, it follows $A'[i,q] = A'[q,j] = 1$. Thereby $intermediaryWalk(i,q,2^{-1}) = intermediaryWalk(q,j,2^{-1}) = ()$, and the resulting walk is $(q)$.

Induction step $k > 0$. Assume the statement is true for $k' = k-1$. Then $intermediaryWalk(i, j, 2^{k-1})$ and $intermediaryWalk(q, j, 2^{k-1})$ produce correct walks, and so is the walk $intermediaryWalk(i,j,2^k)$.

\textbf{Corollary.} Correctness of $main$.
