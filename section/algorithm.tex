...
\textbf{Comment.} The $\min$ is for uniquely choosing a vertex. Any function which selects an element from a subset is valid.

\textbf{Lemma.} $M^{2^k}[i,j] \neq Nil \longleftrightarrow \text{ there is an ij-walk of length at most } 2^k$
...
\textbf{Definition.} An intermediary walk between $i$ and $j$ is
$$
