...
\textbf{Definition.} Let $G$ be a digraph with adjacency matrix $A'$. Let $A = A' + I_n$. The intermediary matrix $M^{2^k}$ for $k \geq 1$ is defined as follows: %corresponding to $A^{[1]}, A^{[2]}, A^{[4]}, \dots, A^{[2^m]}$, where $A^{[1]}$ corresponds to $G \cup G^0$ %and $A^{[2k]} = A^{[k]} \odot A^{[k]}$.
\begin{align*}
% M^1[i,j] &= \begin{cases} 
%     \phi & A^{[1]}[i,j]=1, \; i \neq j \\
%       Nil & \text{otherwise}
...
intermediaryPath(i, j, k)
    ...
    # base case. adjacent, then no intermediary path
    if G[i,j] == 1 return ()
...
# input: k power of 2, at least n-1; i & j
# output: a path from i to j
main(i, j, k)
