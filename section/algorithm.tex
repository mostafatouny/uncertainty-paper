...
\textbf{Definition.} Boolean Vector. A vector whose entries are zero or one.

\textbf{Definition.} Boolean Dot Product. Given boolean vectors $v$ and $w$, the boolean dot product is $v \odot w = \bigvee_k \big ( v[k] \wedge w[k] \big )$, whose co-domain is $1$ and $0$.
