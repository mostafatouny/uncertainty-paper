...
\textbf{Proposition.} If $k < n-1$, and $A^{[k]}[i,j] = 1$, then there is a path from $i$ to $j$ in $G$, but if $A^{[k]}[i,j] = 0$ then the existence of such a path is inconclusive.
