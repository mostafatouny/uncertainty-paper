...
\textbf{Fact.} If $w(v) = 0$ whenever $v$ has no inward edge in $G$, then the weight of path $p$ in $G$ defined as $W(p) = \sum_{v \in p} w(v)$, is equal to $W'(p) = \sum_{e \in p}w(e)$ in $G'$.
