...
% \textbf{Definition (Weight-backward Transformation).} Suppose we are given a digraph $G$ where the ith vertex is labeled with expectation $Ex[X_i]$. Construct a digraph $G'$ by labeling the edges of $G$ as follows: for every $v_i \in V(G)$, we label an edge $(v_k, v_i)$ by $Ex[v_i]$.
...
\textbf{Proposition.} Given a digraph $G$ with fixed vertices $s$ and $d$. Let $L$ and $L'$ be two labels of vertices of $G$. Assume for any $v \in V - \{s,d\}$ where a $sv$-path and $vd$-path do exist, we have $L'(v) = L(v)$, then the shortest $sd$-path under $L$ is also a shortest-path under $L'$.

%\textit{Proof.} Let $\Delta = ((L'(s) - L(s) + (L'(d) - L(d))$ be the net weight change of vertices $s$ and $d$. Then for any path $p_i$, its new weight is $L'(p_i) = L(p_i) + \Delta$. It follows a minimal-weight path $p_k$ is preserved. Indeed, if we have real numbers $a_1, a_2, \dots, a_k$ with minimum $a_k$, then $(a_k + \Delta)$ would be a minimum of $(a_1 + \Delta), \dots, (a_k + \Delta)$.
...
\textit{Proof.} The correctness of memoization is already known. Given a correct memoization, the correctness of the all pairs shortest path is given, since it is used as a black-box subroutine. $\blacksquare$
