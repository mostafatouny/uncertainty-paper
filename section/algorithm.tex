...
\textbf{Definition.} \textit{weight-backward} transformation. We are given a graph $G$ where the ith vertex is labeled with expectation $Ex[X_i]$, and $Ex[X_{root}] = 0$. Construct $G'$ whose edges are labeled as:
$$
Ex[X_{(v_k,v_i)}] = Ex[X_i] \; \text{in} \; G'
\longleftrightarrow
(v_k, v_i) \in E \; \text{in} \; G
$$
...
\textit{Proof.} Recall by definition $Ex[X_{root}] = 0$.

For the base case where $|p| = 1$, total expectation $Ex[X] = 0$ in $G$. In $G'$, $p$ won't have any edges, so $Ex[X] = 0$ in $G'$ as well.
...
Matrix $A$ here is the adjacency of a given graph $G$ whereby $A[i,j] = 1$ if and only if $(v_i, v_j) \in E(G)$. A useful property of adjacency matrices is that $A^k(i,j)$ is exactly the number of distinct walks of length $k$ from vertex $v_i$ to vertex $v_j$. Moreover if the adjacency matrix $A$ has $A[i,i] = 1 \; \forall i$, then $A^k(i,j)$ is the number of walks of length at most $k$ from $v_i$ to $v_j$. See \cite{discreteHandbook} for a background.

Matrix multiplication here, opposing the convention, is by the dot-product, defined as $v_i \cdot v_j = \bigvee_k \big ( (v_i, v_k) \wedge (v_k, v_j) \big )$.

\textbf{Proposition.} For $k < n-1$, if $A^k[i,j] = 1$ then there is a path, but if $A^k[i,j] = 0$ then undecided.
...
\textit{Proof.} By strong induction on the length of paths.
