...
\textbf{Corollary.} If $k < n-1$, and $A^{[k]}[i,j] = 1$, then there is a path from $i$ to $j$ in $G$, but if $A^{[k]}[i,j] = 0$ then the existence of such a path is inconclusive.

\textbf{Definition.} Intermediary matrices $M$ corresponding to $A^{[1]}, A^{[2]}, A^{[4]}, \dots, A^{[2^m]}$, where $A^{[1]} = G \cup G^0$ and $A^{[2k]} = A^{[k]} \odot A^{[k]}$.
\begin{align*}
M^1[i,j] &= \begin{cases} 
      \phi & A^1[i,j]=1, \; i \neq j \\
      Nil & \text{otherwise}
